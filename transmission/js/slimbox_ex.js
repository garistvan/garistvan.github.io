/*
	Slimbox (Extended Version 1.3.1.xdove, 2008-12-30)
	...
	reloadParentonExit() feature added by AdNovea (Laurent)
	on Dec 30th, 2008 for XDove
...
var Lightbox = {
	init: function(options){
		...
		*/
		...
		this.center = new Element('div').setProperty('id', 'lbCenter').setStyles({width: this.options.initialWidth+'px', height: this.options.initialHeight+'px', display: 'none'}).injectInside(document.body);
		
		this.canvas = new Element('div').setProperty('id', 'lbCanvas').injectInside(this.center);
		new Element('a').setHTML(bt_close).setProperties({id: 'lbCloseLink', href: '#'}).injectInside(this.canvas).onclick = this.overlay.onclick = this.close.bind(this);
	...
	close: function(){ 
		...
		if (this.matchOrDefault(rev, new RegExp("reloadParentonExit=(true)", "i"), -1)=="true") 
